...
- `bun test`: 26 pass (12 pre-existing + 14 new: 9 in `golden-rule-guard.test.ts`, 2 new wiring tests in `chat.test.ts`, 1 new visual-check test in `chat-view.test.tsx`, plus 2 `buildSafeFallbackReply` tests). `bun run lint` and `bun run typecheck` both clean.
...
- No new dependencies added; `src/server/golden-rule/guard.ts` is 57 lines, well under the project's 500-line ceiling.
